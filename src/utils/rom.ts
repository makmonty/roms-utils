...
import type { RomExtension, RomLanguage, RomRegion, RomDescription } from '../types/rom.ts';
...
export function getRomDescription(romPath: string) {
  ...
  const regions = tags.filter(tag => nointroRegions.includes(tag as any)) as unknown as RomRegion[];
  const languages = tags.filter(tag => nointroLanguages.includes(tag as any)) as unknown as RomLanguage[];
  ...
  const revision = parseInt(tags.find(tag => tag.startsWith('Rev'))?.match('\d+')?.[0] || '0')
...
export function getPreferenceMatchingRoms(romDescriptions: RomDescription[], pref: ConfigPreference) {
  let matchingRoms = romDescriptions

  for (const item of pref.order) {
    matchingRoms = getPreferenceItemMatchingRoms(matchingRoms, item, pref.type);
...
export function getPreferenceItemMatchingRoms(romDescriptions: RomDescription[], item: ConfigPreference['order']['0'], type: ConfigPreference['type']) {
  return romDescriptions.filter(romDesc => {
    return Array.isArray(romDesc[type]) ? romDesc[type].includes(item) : romDesc[type] === item
  })
}
